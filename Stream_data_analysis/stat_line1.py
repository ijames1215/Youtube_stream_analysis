...
for num in cat:
    ...
    print(avg)
        ###############發片時間點欄位轉換###########
    post_time = []
    for post in range(0,len(data_column)):
        posts = str(post)+'H'
    ...
    plt.plot(post_time[:24], avg,label= num +'  ' + 'category')
    plt.legend(loc='upper right')
    plt.fill_between(post_time[:24],0,avg , facecolor = "lightskyblue", interpolate= True, alpha=0.6)
    plt.ylim((0, 3000))
    plt.title("影片發布流量高峰期",fontproperties=myfont, fontsize=15)
    plt.gcf().savefig('./Final/cat_image/{}/{}.png'.format(num, num + '_line1'),dpi = 100)
    plt.show()
    # fig.savefig(r'./data/stream/movie/stream_{}.png'.format(sss))