...
for num in cat:
    ...
    top6 = [] ;avg = [] ; temp_avg = []
    for sss in list6:
        for int_avg_1_24 in range(1,25):
            for int_avg in range(len(list4)):
                if list4[int_avg][0] == sss:
                    filter = list4[int_avg][int_avg_1_24]
                    if math.isnan(filter) != True:
                        print("第", int_avg_1_24, "時")
                        temp_avg.append(list4[int_avg][int_avg_1_24])
                        pass
                    else:
                        pass
                else:
            ...
            try:
                ...
                temp_avg = []
        try:
            top6.append(int(statistics.mean(avg[3:9])))
            avg = []
        except Exception:
            top6.append(0)
            avg = []
    ...
    plt.ylabel('發片高峰期平均流量(觀看次數)',fontproperties=myfont, fontsize=15)
    plt.plot(list6,top6,label= num +'  ' + 'category')
    plt.legend(loc='upper right')
    plt.fill_between(list6,0,top6 , facecolor = "lightgreen", interpolate= True, alpha=0.5)
    plt.ylim((0, 8000))
    plt.title("主要發片時間點 與 流量高峰期 相對關係",fontproperties=myfont, fontsize=15)
    plt.gcf().savefig('./Final/cat_image/{}/{}.png'.format(num, num + '_line2'))
    plt.show()